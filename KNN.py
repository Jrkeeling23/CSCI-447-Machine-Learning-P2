...
class KNN:
    ...
    def zero_one_loss(self, data_set, k_val, name, in_data):
            ...
            class_col = self.data.get_label_col(self.current_data_set)  # column that class values will be stored in for given dataset
            print("\n-----------------Performing Zero One Loss-----------------")

            # go through each datapoint (row) and check what class KNN thinks it is
            for index, row in data_set.iterrows():

                knn_class = self.perform_knn(row, data_set, k_val,self.current_data_set, self.data)
